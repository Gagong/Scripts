...
function MyHeRoJarvan:Global_Menu()	
	...
	self.Menu:addSubMenu("[Jarvan] - Combo", "Combo")
		...
		self.Menu.Combo:addParam("Cmode", "Combo Q and E Mode:", SCRIPT_PARAM_LIST, 1, {"Smart", "E+Q", "Q or E", "Q Only", "E Only"})
    	self.Menu.Combo:addParam("Burst", "All In/Burst Combo:", SCRIPT_PARAM_ONKEYTOGGLE, false, string.byte("S"))
		...
		self.Menu.Combo:addParam("UseFocus", "Use Cistom Target Selector:", SCRIPT_PARAM_ONOFF, true)
	...
	self.Menu:addSubMenu("[Jarvan] - Harass", "Harass")
		...
		self.Menu.Harass:addParam("Hmode", "Harass Q and E Mode:", SCRIPT_PARAM_LIST, 1, {"E+Q", "Q or E", "Q Only", "E Only"})		
	...
	self.Menu:addSubMenu("[Jarvan] - Last Hit", "LastHit")
		self.Menu.LastHit:addParam("LastHitKey", "Jungle Clear:", SCRIPT_PARAM_ONKEYDOWN, false, string.byte("A"))
	...
	self.Menu:addSubMenu("[Jarvan] - Utility", "Utility")
		...
		self.Menu.Utility:addSubMenu("Item Settings", "Item")
			...
			self.Menu.Utility.Item:addParam("Tiamat", "Use Tiamat/Hydra:", SCRIPT_PARAM_ONOFF, true)
        ...
        if VIP_USER then
   		...
   		if self.SSmite then
   			...
   			self.Menu.Utility.Smite:addSubMenu("Smite Monsters:", "SmiteM")
		        if GetGame().map.shortName == "twistedTreeline" then
		            self.Menu.Utility.Smite.SmiteM:addParam("Wraith", "Use Smite on: Wraith", SCRIPT_PARAM_ONOFF, true)
		            self.Menu.Utility.Smite.SmiteM:addParam("Golem", "Use Smite on: Golem", SCRIPT_PARAM_ONOFF, true)
		            self.Menu.Utility.Smite.SmiteM:addParam("Wolf", "Use Smite on: Wolf", SCRIPT_PARAM_ONOFF, true)
		           	self.Menu.Utility.Smite.SmiteM:addParam("Spiderboss", "Use Smite on: SpiderBoss", SCRIPT_PARAM_ONOFF, true)
		        else
		            self.Menu.Utility.Smite.SmiteM:addParam("Dragon", "Use Smite on: Dragon", SCRIPT_PARAM_ONOFF, true)
		            self.Menu.Utility.Smite.SmiteM:addParam("Baron", "Use Smite on: Baron", SCRIPT_PARAM_ONOFF, true)
		            self.Menu.Utility.Smite.SmiteM:addParam("Razorbeak", "Use Smite on: Wraith", SCRIPT_PARAM_ONOFF, false)
		            self.Menu.Utility.Smite.SmiteM:addParam("Murkwolf", "Use Smite on: Wolf", SCRIPT_PARAM_ONOFF, false)
		            self.Menu.Utility.Smite.SmiteM:addParam("Krug", "Use Smite on: Krug", SCRIPT_PARAM_ONOFF, false)
		            self.Menu.Utility.Smite.SmiteM:addParam("Crab", "Use Smite on: Crab", SCRIPT_PARAM_ONOFF, false)
		            self.Menu.Utility.Smite.SmiteM:addParam("Gromp", "Use Smite on: Gromp", SCRIPT_PARAM_ONOFF, false)
		            self.Menu.Utility.Smite.SmiteM:addParam("Red", "Use Smite on: Red Buff", SCRIPT_PARAM_ONOFF, true)
		            self.Menu.Utility.Smite.SmiteM:addParam("Blue", "Use Smite on: Blue Buff", SCRIPT_PARAM_ONOFF, true)
		        end	
		end
	...
	self.Menu:addSubMenu("[Jarvan] - Draw Settings", "Drawings")
		...
		self.Menu.Drawings:addParam("drawHP", "Draw HP Bar Damage:", SCRIPT_PARAM_ONOFF, true)
...
function MyHeRoJarvan:OnCreateObj(object)
	...
	if object.name:lower():find("beacon") then
		self.EObj = object
		print("E Find")
	end
end
...
function MyHeRoJarvan:OnDeleteObj(object)
	...
	if object ~= nil and object.name:lower():find("beacon") then
		self.EObj = nil
		print("E Remove")
	end
end

function MyHeRoJarvan:BetaE(t)
	local Vec2 = Vector(t.x - myHero.x, t.y - myHero.y, t.z - myHero.z):normalized()
	local lenght = GetDistance(t)*1.5
	local Vec3 = Vec2 * lenght
	if myHero:CanUseSpell(_E) == READY and self.Menu.Combo.ComboMode then
		CastSpell(_E, Vec1.x, Vec1.y, Vec1.z)
	end		
end
	...
	if Smite ~= nil then
	    ...
	    for _, jminions in pairs(minionManager(MINION_JUNGLE, 1000, myHero, MINION_SORT_HEALTH_ASC).objects) do
	        if not jminions.dead and jminions.visible and ValidTarget(jminions, 560) then
	                ...
	                if self.Menu.Utility.Smite.SmiteM[name] then
...
function MyHeRoJarvan:CastQtoE()
	if self.Menu.Combo.ComboMode and self.EObj ~= nil then
		local t = ts.target
		if not t.dead and ValidTarget(t, 1000) then
			if myHero:CanUseSpell(_E) == READY and myHero:CanUseSpell(_Q) == READY then
				self:BetaE(t)
				DelayAction(function() CastSpell(_Q, self.EObj) end, 0.3)
			end
		end
	end
end
...
function MyHeRoJarvan:CastQ(unit)
  	...
  	if VP ~= nil then
    	local CastPosition, HitChance = VP:GetLineCastPosition(unit, self.SpellInfo.Q.Delay, self.SpellInfo.Q.Widrh, self.SpellInfo.Q.Range, self.SpellInfo.Q.Speed, myHero, false)
    	if HitChance >=self.Menu.Prediction.QVPHC then
      		CastSpell(_Q, CastPosition.x, CastPosition.z)
    	end
  	end
  	if DP ~= nil then
    	local state,hitPos,perc = DP:predict(nil, unit, myHero, SkillShot.TYPE.LINE, self.SpellInfo.Q.Delay, self.SpellInfo.Q.Range, self.SpellInfo.Q.Widrh, self.SpellInfo.Q.Delay * 1000, 0, {Minions = false, Champions = false})
    	if perc >=self.Menu.Prediction.QHC then
      		CastSpell(_Q, hitPos.x, hitPos.z)
    	end
  	end
  	if FHPred and Menu.Prediction.CurrentPred == 3 then
  	...
  	if SP ~= nil then
    	local CastPosition, Chance, PredPos = SP:Predict(unit, self.SpellInfo.Q.Range, self.SpellInfo.Q.Speed, self.SpellInfo.Q.Delay, self.SpellInfo.Q.Widrh, false, myHero)
    	if Chance >= self.Menu.Prediction.QSPHC then
      		CastSpell(_Q, CastPosition.x, CastPosition.z)
    	end
  	end
  	if HP ~= nil then
        local Position, HitChance = HPred:GetPredict(HPSkillshot({type = "DelayLine", delay = self.SpellInfo.Q.Delay, range = self.SpellInfo.Q.Range, speed = self.SpellInfo.Q.Speed, collisionH = false, collisionM = false, width = self.SpellInfo.Q.Widrh}), unit, myHero)
    ...
    if KP ~= nil then
        ...
        local CastPosition, hc = KP:GetPrediction(self.KPinfoQ, unit, myHero)
...
function MyHeRoJarvan:CastE(unit)
  	...
  	if VP ~= nil then
    	local CastPosition, HitChance = VP:GetLineCastPosition(unit, self.SpellInfo.E.Delay, self.SpellInfo.E.Widrh, self.SpellInfo.E.Range, self.SpellInfo.E.Speed, myHero, false)
    	if HitChance >= self.Menu.Prediction.EVPHC then
      		CastSpell(_E, CastPosition.x, CastPosition.z)
    	end
  	end
  	if DP ~= nil then
    	local state,hitPos,perc = DP:predict(nil, unit, myHero, SkillShot.TYPE.CIRCLE,self. SpellInfo.E.Speed, self.SpellInfo.E.Range, self.SpellInfo.E.Widrh, self.SpellInfo.E.Delay * 1000, 0, {Minions = false, Champions = false})
  	...
  	if SP ~= nil then
    	local CastPosition, Chance, PredPos = SP:Predict(unit, self.SpellInfo.E.Range, self.SpellInfo.E.Speed, self.SpellInfo.E.Delay, self.SpellInfo.E.Widrh, false, myHero)
    	if Chance >= Menu.Prediction.ESPHC then
      		CastSpell(_E, CastPosition.x, CastPosition.z)
    	end
  	end
  	if HP ~= nil then
        local Position, HitChance = HPred:GetPredict(HPSkillshot({type = "DelayCircle", delay = self.SpellInfo.E.Delay, range = self.SpellInfo.E.Range, speed = self.SpellInfo.E.Speed, collisionH = false, collisionM = false, width = self.SpellInfo.E.Widrh}), unit, myHero)
    ...
    if KP ~= nil then
        ...
        local CastPosition, hc = KP:GetPrediction(self.KPinfoE, unit, myHero)
...
function MyHeRoJarvan:OnDraw()
	...
	if self.Menu.Drawings.AllDraw then	
		if self.Menu.Drawings.drawHP then
	      	self:DrawHPbar()
	    end
...
function MyHeRoJarvan:GetQDamage(unit)
  	self.Qlvl = myHero:GetSpellData(_Q).level
  	if self.Qlvl < 1 then return 0 end
  	self.QDmg = {70, 115, 160, 205, 250}
  	self.QDmgMod = 1.2
  	self.DmgRaw = self.QDmg[self.Qlvl] + myHero.totalDamage * self.QDmgMod
  	self.Dmg = myHero:CalcDamage(unit, self.DmgRaw)
  	return self.Dmg
end

function MyHeRoJarvan:GetEDamage(unit)
  	self.Elvl = myHero:GetSpellData(_E).level
  	if self.Elvl < 1 then return 0 end
  	self.EDmg = {60, 105, 150, 195, 240}
  	self.EDmgMod = 0.8
  	self.DmgRaw = self.EDmg[self.Elvl] + (myHero.ap * self.EDmgMod)
  	self.Dmg = myHero:CalcDamage(unit, self.DmgRaw)
  	return self.Dmg
end

function MyHeRoJarvan:GetRDamage(unit)
  	self.Rlvl = myHero:GetSpellData(_R).level
  	if self.Rlvl < 1 then return 0 end
  	self.RDmg = {200, 325, 450}
  	self.RDmgMod = 1.5
  	self.DmgRaw = self.RDmg[self.Rlvl] + (myHero.totalDamage * self.RDmgMod)
  	self.Dmg = myHero:CalcDamage(unit, self.DmgRaw)
  	return self.Dmg
end

function MyHeRoJarvan:DrawLineHPBar(damage, text, unit, enemyteam)
  	if unit.dead or not unit.visible then return end
  	local p = WorldToScreen(D3DXVECTOR3(unit.x, unit.y, unit.z))
  	if not OnScreen(p.x, p.y) then return end
  	self.thedmg = 0
  	self.line = 2
  	self.linePosA  = {x = 0, y = 0 }
  	self.linePosB  = {x = 0, y = 0 }
  	self.TextPos   = {x = 0, y = 0 }

  	if damage >= unit.health then
    	self.thedmg = unit.health - 1
    	text = "Killable!"
  	else
    	self.thedmg = damage
    	text = "You Damage:"
  	...
  	self.thedmg = math.round(self.thedmg)

  	local StartPos, EndPos = self:GetHPBarPos(unit)
  	local Real_X = StartPos.x + 24
  	local Offs_X = (Real_X + ((unit.health - self.thedmg) / unit.maxHealth) * (EndPos.x - StartPos.x - 2))
  	if Offs_X < Real_X then Offs_X = Real_X end 
  	local number1 = 350 - math.round(255*((unit.health-self.thedmg)/unit.maxHealth))
  	if number1 >= 255 then number1=254 end
  	local number2 = math.round(400*((unit.health-self.thedmg)/unit.maxHealth))
  	if number2 >= 255 then number2=254 end

  	if enemyteam then
    	self.linePosA.x = Offs_X-150
    	self.linePosA.y = (StartPos.y-(30+(self.line*15)))    
    	self.linePosB.x = Offs_X-150
    	self.linePosB.y = (StartPos.y-10)
    	self.TextPos.x = Offs_X-148
    	self.TextPos.y = (StartPos.y-(30+(self.line*15)))
  	else
    	self.linePosA.x = Offs_X-125
    	self.linePosA.y = (StartPos.y-(30+(self.line*15)))    
    	self.linePosB.x = Offs_X-125
    	self.linePosB.y = (StartPos.y-15)
    	self.TextPos.x = Offs_X-122
    	self.TextPos.y = (StartPos.y-(30+(self.line*15)))
  	end

  	DrawLine(self.linePosA.x, self.linePosA.y, self.linePosB.x, self.linePosB.y , 2, ARGB(number1, 255, number2, 0))
  	DrawText(tostring(self.thedmg).." "..tostring(text), 15, self.TextPos.x, self.TextPos.y , ARGB(number1, 255, number2, 0))
end

function MyHeRoJarvan:DrawHPbar()
  	for i, HPbarEnemyChamp in pairs(GetEnemyHeroes()) do
    	if not HPbarEnemyChamp.dead and HPbarEnemyChamp.visible then
      		self.dmg = myHero:CalcDamage(HPbarEnemyChamp, myHero.totalDamage)
      		if myHero:CanUseSpell(_Q) == READY == READY and not HPbarEnemyChamp.dead then
        		self.dmg = self.dmg + self:GetQDamage(HPbarEnemyChamp)
      		end
	      	if myHero:CanUseSpell(_E) == READY == READY and not HPbarEnemyChamp.dead then
	        	self.dmg = self.dmg + self:GetEDamage(HPbarEnemyChamp)
	      	end
	      	if myHero:CanUseSpell(_R) == READY == READY and not HPbarEnemyChamp.dead then
	      		self.dmg = self.dmg + self:GetRDamage(HPbarEnemyChamp)
	      	end
	      	if Ignite ~= nil then
	        	if myHero:CanUseSpell(Ignite) then
	          		self.dmg = self.dmg + (50 + 20*myHero.level)
	        	end
	      	end
      		self:DrawLineHPBar(self.dmg, "", HPbarEnemyChamp, HPbarEnemyChamp.team)
    	end
  	end
end

function MyHeRoJarvan:GetHPBarPos(enemy)
