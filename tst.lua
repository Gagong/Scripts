...
--Credits SxTeam  ]]-- 
    ...
    Menu:addParam('GameVer', '6.6.136.4261 <Releases/6.6>', SCRIPT_PARAM_INFO, "")
...
function LevelUp()
	if Menu.Enable or Menu.AlwaysON then
		if Menu.Mode == 1 then
        ...
        if myHero.level == 1 then
            DelayAction(function() PrintChat("<font color='#00BFFF'>This Level: </font><font color='#7CFC00'>"..Skills[Sequence[myHero.level]].."</font><font color='#00BFFF'><font color='#FF0000'> ===></font> Next level: </font><font color='#7CFC00'>"..Skills[Sequence[myHero.level + 1]].. "</font><font color='#EE82EE'>. </font>") end, 9)
        end

		if myHero.level > 1 and myHero.level < 18 then
...
function OnDraw()
    --if myHero.charName == CustomChar then
        DrawText("You Need Chose Custom Sequence", 20, 500, 500, 0xFFFFF5EE)
    --end
end
