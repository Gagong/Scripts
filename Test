...
---1.2 - 6.1 Updated---
---1.3 - Added SpellUsage for more Champions---
...
function HSMenuInit()
   ...
   DelayAction(function() HSMenu:permaShow("Intro") end, 5.0)
...
function GetDamageSpell()
	...
	if myHero.charName == "MasterYi" then
		QLevel = myHero:GetSpellData(_Q).level
	if QLevel == 1 then
		SpellDamage = 100 + (1*myHero.ap)
	elseif QLevel == 2 then
		SpellDamage = 160 + (1*myHero.ap)
	elseif QLevel == 3 then
		SpellDamage = 220 + (1*myHero.ap)
	elseif QLevel == 4 then
		SpellDamage = 280 + (1*myHero.ap)
	elseif QLevel == 5 then
		SpellDamage = 340 + (1*myHero.ap)
	end
	end
...
function OnDraw()
	...
	if HSMenu.Draw.drawSmite and myHero:CanUseSpell(Smite) == READY and HSMenu.SmiteActive then
		DrawCircle(myHero.x, myHero.y, myHero.z, 550, RGB(100, 44, 255))
			self.drawDamage = obj.health - GetSmiteDamage - GetDamageSpell
		if minion.health > GetSmiteDamage then
			DrawText3D(tostring(math.ceil(self.drawDamage)),mousePos.x, mousePos.y, mousePos.z, 24, 0xFFFF0000)
		else
			DrawText3D("Smitable", mousePos.x, mousePos.y, mousePos.z, 24, 0xFFFF0000)
		end
	end
end
